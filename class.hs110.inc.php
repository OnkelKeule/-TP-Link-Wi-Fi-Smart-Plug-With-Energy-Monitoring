...
class HS110
{
	...
	public function on(){
		...
		$param='{"system":{"set_relay_state":{"state":1}}}';
		$this->socket($param);
	...
	public function off(){
		...
		$param='{"system":{"set_relay_state":{"state":0}}}';
		$this->socket($param);
	...
	public function info($item=NULL){
		...
		$param='{"system":{"get_sysinfo":{}}}';
		$return=$this->socket($param);

		if(!is_null($item)){
			return $return['system']['get_sysinfo'][$item];
		}
		else{
			return $return['system']['get_sysinfo'];
		}

	}
	private function tp_decrypt ( $cypher_text , $first_key = 0xAB ) {
	...
	private function tp_encrypt ( $clear_text , $first_key = 0xAB ) {
	...
	public function socket($message){
		$this->request=$message;
		$message=$this->tp_encrypt($message);
		...
		// echo $buf;
		$this->result=json_decode($this->tp_decrypt($buf));
		return json_decode($this->tp_decrypt($buf), true);
	}
